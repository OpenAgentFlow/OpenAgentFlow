...
def get_llm(model: Optional[str] = "gemma-4-26b-a4b-it", temperature: float = 0.3, provider: Optional[str] = None):
...
def analyst_node(state: WorkflowState) -> WorkflowState:
    """Agent: Analyst"""
    llm = get_llm(model="gemma-4-26b-a4b-it", temperature=0.3, provider="gemini")
    print(f'[Analyst] Running agent (model="gemma-4-26b-a4b-it", provider={getattr(llm, "_oaf_provider", "unknown")})')

    system_prompt = """Analyze the requirements document.
...
Produce a structured analysis."""
    ...
    if state.get("requirements") is not None:
        user_parts.append(f"requirements: {state['requirements']}")
    ...
    return _parse_llm_output(response.content, ["analysis"])


def architect_node(state: WorkflowState) -> WorkflowState:
    """Agent: Architect"""
    llm = get_llm(model="gemma-4-26b-a4b-it", temperature=0.5, provider="gemini")
    print(f'[Architect] Running agent (model="gemma-4-26b-a4b-it", provider={getattr(llm, "_oaf_provider", "unknown")})')

    system_prompt = """Based on the analysis, design a technical architecture.
Include component diagrams, data flow, and API contracts."""

    # Collect input from state
    user_parts = []
    if state.get("analysis") is not None:
        user_parts.append(f"analysis: {state['analysis']}")
    user_message = "\n".join(user_parts) if user_parts else "No input provided."
    ...
    response = llm.invoke(messages)
    return _parse_llm_output(response.content, ["architecture"])


def developer_node(state: WorkflowState) -> WorkflowState:
    """Agent: Developer"""
    llm = get_llm(model="gemma-4-26b-a4b-it", temperature=0.2, provider="gemini")
    print(f'[Developer] Running agent (model="gemma-4-26b-a4b-it", provider={getattr(llm, "_oaf_provider", "unknown")})')

    system_prompt = """Implement the solution based on the architecture.
Write clean, tested, production-ready code.\n\nIMPORTANT: You must respond ONLY with a valid JSON object containing exactly these fields: implementation, status. Do not include any other text, markdown, or commentary outside the JSON object."""

    # Collect input from state
    user_parts = []
    if state.get("architecture") is not None:
        user_parts.append(f"architecture: {state['architecture']}")
    user_message = "\n".join(user_parts) if user_parts else "No input provided."
    ...
    response = llm.invoke(messages)
    return _parse_llm_output(response.content, ["implementation","status"])
...
if __name__ == "__main__":
    ...
    print(f"Running workflow: Software Development")
    print(f"{'-' * 50}")
