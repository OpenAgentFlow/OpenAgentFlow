...
class WorkflowState(TypedDict, total=False):
    customer_email: Optional[str]  # @required
...
def get_llm(model: Optional[str] = "gemini-3.1-flash-lite", temperature: float = 0.1, provider: Optional[str] = None):
...
def classifier_node(state: WorkflowState) -> WorkflowState:
    """Agent: Classifier"""
    llm = get_llm(model="gemini-3.1-flash-lite", temperature=0.1, provider="gemini")
    print(f'[Classifier] Running agent (model="gemini-3.1-flash-lite", provider={getattr(llm, "_oaf_provider", "unknown")})')

    system_prompt = """Analyze the inbound customer_email.
Determine the issue category (e.g., Billing, TechSupport, FeatureRequest).
Assign an urgency score from 1 (routine) to 5 (critical escalation).    \n\nIMPORTANT: You must respond ONLY with a valid JSON object containing exactly these fields: categories, urgency. Do not include any other text, markdown, or commentary outside the JSON object."""

    # Collect input from state
    user_parts = []
    if state.get("customer_email") is not None:
        user_parts.append(f"customer_email: {state['customer_email']}")
    ...
    return _parse_llm_output(response.content, ["categories","urgency"])


def drafter_node(state: WorkflowState) -> WorkflowState:
    """Agent: Drafter"""
    llm = get_llm(model="gemma-4-26b-a4b-it", temperature=0.7, provider="gemini")
    print(f'[Drafter] Running agent (model="gemma-4-26b-a4b-it", provider={getattr(llm, "_oaf_provider", "unknown")})')

    system_prompt = """Write a polite, empathetic response addressing the customer's issue.
...
Leave placeholders like [Your Name] for account details."""
    ...
    if state.get("categories") is not None:
        user_parts.append(f"categories: {state['categories']}")
    if state.get("urgency") is not None:
        user_parts.append(f"urgency: {state['urgency']}")
    user_message = "\n".join(user_parts) if user_parts else "No input provided."
    ...
    response = llm.invoke(messages)
    return _parse_llm_output(response.content, ["draft_response"])
...
if __name__ == "__main__":
    ...
    print(f"Running workflow: SupportTriage")
    print(f"{'-' * 50}")
