...
class WorkflowState(TypedDict, total=False):
    raw_data: Optional[str]  # @required
    cleaned_data: Optional[str]
    validation_errors: Optional[str]
    is_valid: Optional[bool]
    processing_attempts: Annotated[Optional[int], operator.add]
...
def cleaner_node(state: WorkflowState) -> WorkflowState:
    ...
    print(f'[Cleaner] Running agent (model="gemma-4-26b-a4b-it", provider={getattr(llm, "_oaf_provider", "unknown")})')
...
Output the result as cleaned_data and increment processing_attempts by 1.\n\nIMPORTANT: You must respond ONLY with a valid JSON object containing exactly these fields: cleaned_data, processing_attempts. Do not include any other text, markdown, or commentary outside the JSON object."""

    # Collect input from state
    user_parts = []
    if state.get("raw_data") is not None:
        user_parts.append(f"raw_data: {state['raw_data']}")
    if state.get("cleaned_data") is not None:
        user_parts.append(f"cleaned_data: {state['cleaned_data']}")
    ...
    return _parse_llm_output(response.content, ["cleaned_data","processing_attempts"])


def validator_node(state: WorkflowState) -> WorkflowState:
    """Agent: Validator"""
    llm = get_llm(model="gemma-4-26b-a4b-it", temperature=0.7, provider="gemini")
    print(f'[Validator] Running agent (model="gemma-4-26b-a4b-it", provider={getattr(llm, "_oaf_provider", "unknown")})')

    system_prompt = """Validate the cleaned_data.
...
If it's invalid, set is_valid to false and describe the issues in validation_errors.\n\nIMPORTANT: You must respond ONLY with a valid JSON object containing exactly these fields: is_valid, validation_errors. Do not include any other text, markdown, or commentary outside the JSON object."""
    ...
    response = llm.invoke(messages)
    return _parse_llm_output(response.content, ["is_valid","validation_errors"])
...
def build_graph() -> StateGraph:
    ...
    def route_Validator(state: WorkflowState) -> str:
        if ((state.get("is_valid") == False) and (state.get("processing_attempts") < 3)): return "Cleaner"
        if ((state.get("is_valid") == True) or (state.get("processing_attempts") >= 3)): return END
        raise ValueError(f"No matching conditional edge from 'Validator'")
...
if __name__ == "__main__":
    ...
    print(f"Running workflow: Data Cleaning Pipeline")
    print(f"{'-' * 50}")
