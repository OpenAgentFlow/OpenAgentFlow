...
| [feedback-analysis.oaf](#4-feedback-analysis--multi-output-pipeline) | 3 | Multi-output agents, JSON parsing, e2e demo |
...
## 4. Feedback Analysis — Multi-Output Pipeline

**File:** `examples/e2e-demo/feedback-analysis.oaf`

A production-style customer feedback analysis workflow demonstrating multi-output agents and JSON parsing.
...
```mermaid
graph LR
    S((start)) --> SA["SentimentAnalyzer<br/>temp: 0.1"]
    SA --> C["Categorizer<br/>temp: 0.2"]
    C --> RD["ResponseDrafter<br/>temp: 0.7"]
    RD --> E((end))
```

### What This Demonstrates

- **Multi-output agent** — Categorizer outputs both `category` and `key_issues`
- **JSON parsing** — when an agent has multiple outputs, the generated code attempts to parse the LLM response as JSON
- **Progressive temperature** — low for classification (0.1), medium for categorization (0.2), high for creative writing (0.7)
- **Multi-input agent** — ResponseDrafter reads 4 state variables
- **Accumulating context** — each agent adds to the shared state, enabling downstream agents to make better decisions

### Multi-Output JSON Behavior

When the Categorizer agent has `outputs: [category, key_issues]`, the generated Python code:

1. Asks the LLM to return a response
2. Attempts `json.loads(result)` to parse it as JSON
3. If successful, extracts `category` and `key_issues` from the parsed object
4. If JSON parsing fails, falls back to assigning the raw text to the first output field (`category`)
